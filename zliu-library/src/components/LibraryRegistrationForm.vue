...
const submitForm = () => {
  ...
  validateReason(true)
  if (!errors.value.username && !errors.value.password && !errors.value.confirmPasswordpassword && !errors.value.reason) {
...
const validatePassword = (blur) => {
 ...
 const validateConfirmPassword = (blur) => {
  ...
  if (formData.value.reason.length < 10) {
    if (blur) errors.value.reason = 'Reason must be 10 characters.'
  } else {
